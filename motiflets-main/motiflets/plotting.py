...
class Motiflets:
    ...
    def fit_k_elbow(
        ...
        """Plots the elbow-plot for k-Motiflets.
            ...
            forbidden_area: list of tuples, optional
            forbidden_area: optional list of (start, end) intervals to exclude
...
def plot_motifset(
        ...
        if motifsets is not None:
            for i, motifset in enumerate(motifsets_sampled):
                if motifset is not None:
                    ...
                    motif_color = sns.color_palette("tab10")[
  ...
  # 绘制 motif 开始和结束位置的虚线
                        start_pos = data_index_sampled[pos]
                        end_pos = data_index_sampled[min(pos + motif_length_sampled - 1, len(data_index_sampled) - 1)]
  ...
  # 添加文本标签显示开始和结束时间（仅对第一个 motif 实例添加，避免标签过多）
                        if a == 0:
  # 开始时间标签
                            axes[0, 0].text(
                                start_pos,
                                offset + (np.max(dim_raw_sampled) - np.min(dim_raw_sampled)) * 0.1,
                                f'Start: {start_pos:.1f}',
                                color=motif_color,
                                fontsize=9,
                                ha='left',
                                va='bottom',
                                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.7, edgecolor=motif_color)
                            )
  # 结束时间标签
                            axes[0, 0].text(
                                end_pos,
...
def plot_elbow(
        ...
        backend="scalable",
        forbidden_area=None  # 禁区columns表，格式为 [(start1, end1), (start2, end2), ...]，None 表示无禁区
):
    """Plots the elbow-plot for k-Motiflets.
