...
@njit(nogil=True, fastmath=True, cache=False, parallel=True)
...
def search_k_motiflets_elbow(
        ...
        distance_preprocessing=sliding_mean_std,
        backend="scalable",
        forbidden_area=None  # list of (start, end); None = no forbidden regions
):
  ...
  # forbidden area：将禁区的子序columns在distance矩阵中标记为无效
        if forbidden_area is not None and len(forbidden_area) > 0:
  # 创建子序column index掩码，标记哪些子序columns与禁区相交
            forbidden_indices = set()
            for forbidden_start, forbidden_end in forbidden_area:
  # 确保区间是有效的
                forbidden_start = max(0, int(forbidden_start))
                forbidden_end = min(data_raw.shape[-1], int(forbidden_end))
  # 遍历所有可能起始的子序column index
                for subseq_start in range(n):
                    subseq_end = subseq_start + m

                    if not (subseq_end <= forbidden_start or subseq_start >= forbidden_end):
                        forbidden_indices.add(subseq_start)
            
  # 将禁区的子序columns在distance矩阵中的行和columns都设置为 inf
            forbidden_indices_list = list(forbidden_indices)
            if len(forbidden_indices_list) > 0:
                forbidden_indices_array = np.array(forbidden_indices_list, dtype=np.int32)
                print(f"[Forbidden Area] masked {len(forbidden_indices_array)} subsequence indices")
                
  # 对于全矩阵 backend（default 或 sparse），可以直接修改 D_full
                if backend in ["default", "sparse"] and hasattr(D_full, 'shape') and len(D_full.shape) == 2:
                    try:
  # 将禁区的行设置为 inf
                        D_full[forbidden_indices_array, :] = np.inf
  # 将禁区的columns设置为 inf
                        D_full[:, forbidden_indices_array] = np.inf
                    except Exception as e:
                        print(f"[Forbidden Area] warning: could not patch distance matrix: {e}")
                
  # 对于所有 backend，都需要修改 knns，确保禁区的子序columns不会被选为 k-NN
                for idx in forbidden_indices_array:
                    if 0 <= idx < knns.shape[0]:
  # 将 knns 中指向禁区的索引设置为 -1
                        knns[idx, :] = -1
  # 同时，将其他子序columns的 knns 中指向这个禁区的索引也设置为 -1
